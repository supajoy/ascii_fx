<script>
  ...
  let useCustomFrame = $derived($config.useCustomFrame);
  let frameWidth = $derived($config.frameWidth);
  let frameHeight = $derived($config.frameHeight);

  function upload() { window.dispatchEvent(new CustomEvent('trigger-upload')); noSource = false; playClick(); }
  function clearMoves() { window.dispatchEvent(new CustomEvent('clear-moves')); playClick(); }
  function clearSourceBtn() { window.dispatchEvent(new CustomEvent('clear-source')); noSource = true; playClick(); }
    ...
    {#if tab === 'style'}
      <Section title="Tools">
        <div class="btn-grid">
          ...
          <button onclick={clearSourceBtn}>No Source</button>
        </div>
      </Section>
      ...
      {#if noSource}
        <Section title="Frame Size">
          <button class:active-tool={useCustomFrame} class:toggled-off={!useCustomFrame}
            onclick={() => { config.update(c => ({ ...c, useCustomFrame: !c.useCustomFrame })); debouncedCommit(); playClick(); }}>
            Custom: {useCustomFrame ? 'ON' : 'OFF'}
          </button>
          {#if useCustomFrame}
            <div class="ctrl-row">
              <span class="ctrl-label">W</span>
              <input type="range" min={200} max={3840} step={10} value={frameWidth} oninput={(e) => set('frameWidth', e)} />
              <input type="number" class="ctrl-num" min={200} max={3840} value={frameWidth} oninput={(e) => set('frameWidth', e)} />
            </div>
            <div class="ctrl-row">
              <span class="ctrl-label">H</span>
              <input type="range" min={200} max={2160} step={10} value={frameHeight} oninput={(e) => set('frameHeight', e)} />
              <input type="number" class="ctrl-num" min={200} max={2160} value={frameHeight} oninput={(e) => set('frameHeight', e)} />
            </div>
          {/if}
        </Section>
      {/if}
